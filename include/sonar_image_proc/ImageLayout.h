...
// Convert the range-major working layout back to the standards-facing
// marine_acoustic_msgs beam-major wire order.
  inline bool rangeMajorToBeamMajor(
    const std::vector < std::uint8_t > &input,
    std::size_t range_count,
    std::size_t beam_count,
    std::size_t bytes_per_cell,
    std::vector < std::uint8_t > &output)
  {
    ...
    for (std::size_t range = 0; range < range_count; ++range) {
      for (std::size_t beam = 0; beam < beam_count; ++beam) {
        const std::size_t src = (range * beam_count + beam) * bytes_per_cell;
        const std::size_t dst = (beam * range_count + range) * bytes_per_cell;
        std::copy_n(input.data() + src, bytes_per_cell, output.data() + dst);
      }
    }
    return true;
